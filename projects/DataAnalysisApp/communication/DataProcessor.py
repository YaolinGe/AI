...
class DataProcessor:
    ...
    def preprocess(self, df: pd.DataFrame) -> tuple:
        ...
        # s2, append back the columns
        df['InCut'] = df_initial['InCut']
        df['Anomaly'] = df_initial['Anomaly']
        ...
        # s5, calculate indices
        raw_df_in_cut = df[(df['InCut'] == 1)]
        raw_df_in_cut = raw_df_in_cut.drop(["InCut"], axis=1)
        raw_df_in_cut.sort_index(inplace=True)
        ...
        # s6, prepare data for LSTM and classical models
        ...
        return [X_train, y_train, X_val, y_val, X_test, y_test], [X_train_LSTM, y_train_LSTM, X_val_LSTM, y_val_LSTM, X_test_LSTM, y_test_LSTM]
